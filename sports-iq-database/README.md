...
```terminal
docker build -t sports-iq-db:latest .
```
...
```bash
sqlpackage /Action:Publish /SourceFile:bin/Debug/SportsIQ.Database.dacpac /TargetServerName:localhost /TargetDatabaseName:SportsIQ.Database
```
